import{s as e}from"./supabaseClient-IcS1GkpV.js";/* empty css                    */import{F as s}from"./loader-tWLxx4qH.js";async function n(){const o=document.getElementById("product-grid"),{data:{user:d}}=await e.auth.getUser();d||s("You must be logged in to view your wishlist");const{data:c,error:i}=await e.from("wishlist").select("product_id");if(i){s("Error fetching wishlist:",i.message);return}const m=c.map(l=>l.product_id),{data:r,error:u}=await e.from("products").select("*").in("id",m);if(!r||r.length===0){o.innerHTML='<p class="dark:text-white text-gray-500 text-lg">Nothing to see here</p>';return}u?s(errormessage):(o.innerHTML=r.map(t=>`
